...
class QuestAndServerTests(unittest.TestCase):
    ...
    def test_server_state_party_chat_and_tick(self):
        ...
        server.create_party("p1", "alpha")
        server.apply_command("p1", "chat", {"text": "hello"})
        ...
        self.assertIn("p1", server.state.parties["alpha"])
        self.assertEqual(server.state.chat_log[-1].text, "hello")
